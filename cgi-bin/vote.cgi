...
try:
    ...
    if len(form) != 0:
        ...
        subprocess.check_output(
            [PATH_TO_MACHINE, 'vote', form.getvalue('voterId'), str(convert_date_to_id(ids[0])), unique_office_id, unqiue_candidate_id,form.getvalue('voterName')])
    ...
    print('<b>Error with subprocess ballot:</b>')
    print('<code>')
    print(e.output.decode('utf-8'), end="")
    print('</code>')
except Exception as e:
    print('<b>Error with main process ballot:</b>')
    print('<code>')
    print(e)
    print('</code>')
